...
App<AppOption>({
  ...
  // 保存设置
  saveSettings(settings: Partial<AppOption['globalData']['settings']>) {
    this.globalData.settings = { ...this.globalData.settings, ...settings };
    Storage.set('settings', this.globalData.settings);
  },
  ...
  // 保存用户信息
  saveUserInfo(userInfo: WechatMiniprogram.UserInfo) {
