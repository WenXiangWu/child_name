...
class NameGenerator {
     ...
     * @returns 适合的汉字列表
     */
    static getSuitableChars(params) {
        ...
        if (meaningKeywords && meaningKeywords.length > 0) {
            const filteredByMeaning = suitableChars.filter(char => {
                ...
                return meaningKeywords.some(keyword => meanings.some(meaning => meaning.includes(keyword)));
            });
     ...
     * @returns 名字分析结果
     */
    static analyzeNaming(familyName, characters, params) {
        // 五行分析
     ...
     * @returns 五行分数
     */
    static calculateFiveElementsScore(elements, params) {
        // 简化的五行评分算法
        let score = 70; // 基础分
        // 如果有期望的五行，且全部符合，加分
     ...
     * @returns 发音分数
     */
    static calculatePronunciationScore(characters) {
        let score = 70; // 基础分
        // 检查声调是否和谐
     ...
     * @returns 字义分数
     */
    static calculateMeaningScore(characters, params) {
        let score = 70; // 基础分
        // 检查是否包含期望的字义关键词
     ...
     * @returns 五行描述
     */
    static getFiveElementsDescription(elements) {
        const counts = {
            '金': 0,
            '木': 0,
            '水': 0,
        ...
        for (const element in counts) {
            if (counts[element] > 0) {
                descriptions.push(`${element}(${counts[element]})`);
            }
        }
     ...
     * @returns 发音描述
     */
    static getPronunciationDescription(characters) {
     ...
     * @returns 字义描述
     */
    static getMeaningDescription(characters) {
        // 提取所有字义
        const allMeanings = [];
     ...
     * @returns 随机正面形容词
     */
    static getRandomPositiveAdjective() {
        const adjectives = [
     ...
     * @returns 随机正面短语
     */
    static getRandomPositivePhrase() {
        const phrases = [
