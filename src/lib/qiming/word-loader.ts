...
export class WordDataLoader {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      const response = await fetch(getStaticUrl('word-index.json'));
  ...
  private async loadChunk(chunkIndex: number): Promise<WordRecord[]> {
    ...
    try {
      const response = await fetch(getStaticUrl(`word-chunks/chunk-${chunkIndex.toString().padStart(3, '0')}.json`));
