...
Page({
  data: {
    ...
    strokeCount: 5,
    strokeRange: [1, 10],
    selectedElement: '' as FiveElement | '',
  ...
  // 选择拼音
  selectPinyin(e: WechatMiniprogram.TouchEvent) {
  ...
  // 选择部首
  selectRadical(e: WechatMiniprogram.TouchEvent) {
    const radical = e.currentTarget.dataset.radical as string;
  ...
  // 笔画数变化
  strokeCountChange(e: WechatMiniprogram.SliderChange) {
    const strokeCount = e.detail.value as number;
  ...
  // 输入最小笔画
  inputMinStroke(e: WechatMiniprogram.Input) {
  ...
  // 输入最大笔画
  inputMaxStroke(e: WechatMiniprogram.Input) {
  ...
  // 选择五行属性
  selectElement(e: WechatMiniprogram.TouchEvent) {
  ...
  // 查看汉字详情
  viewCharDetail(e: WechatMiniprogram.TouchEvent) {
    const char = e.currentTarget.dataset.char as Character;
