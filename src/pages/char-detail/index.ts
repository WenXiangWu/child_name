...
Page({
  ...
  onLoad() {
    ...
    // 监听参数传递
    eventChannel.on('charData', (char: Character) => {
      this.setData({
        charData: char
      });
  ...
  loadPhrases(char: string) {
    ...
    if (char === '德') {
      mockPhrases.push(
        ...
        { phrase: '德高望重', pinyin: 'dé gāo wàng zhòng' },
        { phrase: '美德', pinyin: 'měi dé' }
      );
    } else if (char === '智') {
      mockPhrases.push(
        { phrase: '智慧', pinyin: 'zhì huì' },
        { phrase: '智力', pinyin: 'zhì lì' },
        { phrase: '智商', pinyin: 'zhì shāng' },
        { phrase: '聪明才智', pinyin: 'cōng míng cái zhì' },
        { phrase: '足智多谋', pinyin: 'zú zhì duō móu' }
      );
    } else {
      // 默认词组
      mockPhrases.push(
        { phrase: char + '字', pinyin: '' },
        { phrase: '好' + char, pinyin: '' }
      );
    }
  ...
  // 获取结构分析
  getStructureAnalysis(char: string): string {
    // 简单示例，实际应该有更复杂的分析
