...
Page({
    ...
    onLoad() {
        ...
        // 监听参数传递
        eventChannel.on('nameGenerationParams', (params) => {
            // 更新表单数据
            this.setData({
    ...
    generateNames(params) {
        ...
        // 调用名字生成服务
        setTimeout(() => {
            const nameList = nameGenerator_1.NameGenerator.generateNames(params);
            this.setData({
                nameList,
                loading: false
            });
            ...
            if (nameList.length > 0) {
                this.selectName({ currentTarget: { dataset: { index: 0 } } });
            }
        }, 1500); // 模拟网络延迟
    },
    // 重新生成名字
    ...
    saveName() {
        ...
        try {
            // 获取已收藏的名字
            const favoriteNames = storage_1.Storage.get('favoriteNames') || [];
            // 检查是否已收藏
            ...
            storage_1.Storage.set('favoriteNames', favoriteNames);
            wx.showToast({
                title: '收藏成功',
                icon: 'success'
            });
